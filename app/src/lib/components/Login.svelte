<script>
  ...
  async function handleLogin() {
    if (!phone || !pin) {
      errorMsg = 'Nomor HP dan PIN wajib diisi';
      return;
    }
    ...
    // Simulasi Login (Akan dihubungkan ke database Supabase nanti)
    setTimeout(() => {
      loading = false;
...
<div class="flex-grow flex flex-col justify-between p-8 bg-white h-full">
  ...
  <div>
    ...
    <p class="text-sm text-slate-500 mt-2 leading-relaxed">Masukkan nomor WhatsApp dan PIN keamanan Anda untuk mengakses Buku Saku OB.</p>
    ...
    <div class="mt-8 flex flex-col gap-4">
      ...
      <div>
        <label for="pin" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">4-Digit PIN</label>
        <input 
          type="password" 
          id="pin" 
          inputmode="numeric"
